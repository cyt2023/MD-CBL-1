    ...
    executive.write_text(
        "\n".join(
            [
                "# Executive Summary",
                "",
                "This project evaluates three charging strategies for a shared EV fleet in Eindhoven: baseline, smart priority, and agentic LLM.",
                "",
    ...
    technical.write_text(
        "\n".join(
            [
                "# Technical Summary",
                "",
                "The pipeline loads the Eindhoven mobility, hub, congestion, load, and electricity price datasets, builds a synthetic but reproducible fleet, and runs three comparable 24-hour simulations from the same initial state.",
                "",
                "The baseline scheduler uses a fixed threshold. The smart priority scheduler uses deterministic weighted scoring. The agentic LLM scheduler uses deterministic candidate ranking, an optional LLM planning loop, strict validation and repair, heuristic fallback, and trace logging.",
                "",
